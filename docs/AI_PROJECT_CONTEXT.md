...
> **Версия**: 1.1
> **Назначение**: Промпт для быстрого входа в контекст проекта (Актуализировано после Фазы 2)
...
- [x] **Безопасность**: Проведена полная очистка истории Git от утекших секретов (Postgres, OpenAI, TG). История переписана и принудительно отправлена в репозиторий (`push --force`).

### В процессе / Следующие шаги:
- [ ] **Фаза 3: Система уведомлений** — Автоматические напоминания о ТО за 30 дней.
